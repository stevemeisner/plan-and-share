...
export function Sidebar({ onClose, collapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <aside
      className={`${collapsed ? "w-14" : "w-[var(--sidebar-width)]"} bg-[var(--sidebar-bg)] border-r border-[var(--sidebar-border)] flex flex-col h-screen transition-all duration-200 overflow-hidden`}
    >
      {/* Logo */}
      <div className="p-4 border-b border-[var(--sidebar-border)] flex items-center justify-between">
        <Link to="/" className="text-[var(--plan-text-heading)] font-semibold text-lg flex items-center gap-2">
          <span className="bg-[var(--plan-accent)] text-white w-6 h-6 rounded-md flex items-center justify-center text-xs font-bold shrink-0">P</span>
          {!collapsed && "PlanShare"}
        </Link>
        {collapsed ? (
          <button
            onClick={onToggleCollapse}
            className="hidden lg:block p-1 rounded hover:bg-[var(--plan-bg-hover)] text-[var(--plan-text-muted)]"
      ...
      <nav className="flex-1 overflow-y-auto p-2">
        {collapsed ? (
          <>
            ...
            {/* Collapsed: folder first letters */}
            {folders?.map((folder) => (
              <Link
                key={folder._id}
                to={`/f/${folder.slug}`}
                title={folder.name}
                className={`flex items-center justify-center w-8 h-8 mx-auto my-1 rounded-md text-sm font-medium transition-colors ${
          ...
          <>
            ...
            {pendingReviews && pendingReviews.length > 0 ? (
              ...
              <div className="px-3 py-1 text-xs text-[var(--plan-text-muted)] italic">
                No pending reviews
              </div>
            )}
            <div className="border-b border-[var(--sidebar-border)] my-2" />
